...
final class FoodPortionLifecyclePolicy implements CatalogLifecyclePolicy
{
    public function evaluate(CatalogLifecycleCommand $command, CatalogLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
        if ($command->subjectType !== CatalogLifecycleSubjectType::Portion
            || ! $snapshot instanceof FoodPortionLifecycleSnapshot
    ...
    private function createDraft(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
        if ($snapshot->exists) {
            return CatalogLifecycleResult::invalid(CatalogLifecycleReason::InvalidTransition, $snapshot->state);
        }
    ...
    private function editDraft(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function submit(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
        if ($snapshot->state !== CatalogLifecycleState::Draft) {
            return CatalogLifecycleResult::invalid(CatalogLifecycleReason::InvalidTransition, $snapshot->state);
        }
    ...
    private function approve(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function publish(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function activate(FoodPortionLifecycleSnapshot $snapshot, CatalogLifecycleState $requiredState): CatalogLifecycleResult
    {
        ...
        if ($snapshot->hasActivePortionConflict) {
            return CatalogLifecycleResult::conflict(CatalogLifecycleReason::ActivePortionConflict, $snapshot->state);
        }
    ...
    private function withdraw(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function archive(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function createSuccessor(FoodPortionLifecycleSnapshot $snapshot): CatalogLifecycleResult
    {
    ...
    private function reviewReasons(FoodPortionLifecycleSnapshot $snapshot): array
    {
        $reasons = [];
        if ($snapshot->parentArchived) {
        ...
        if (! $snapshot->localePresent) {
            $reasons[] = CatalogLifecycleReason::PortionLocaleMissing;
        }
        if (! $snapshot->provenanceComplete) {
            $reasons[] = CatalogLifecycleReason::ProvenanceIncomplete;
        }
        if (! $snapshot->hasPositiveUnitQuantity || ! $snapshot->hasPositiveGramWeight || ! $snapshot->preparationApplicabilityValid) {
            $reasons[] = CatalogLifecycleReason::PortionEvidenceInvalid;
        }
    ...
    /** @return list<CatalogLifecycleReason> */
    private function activationReasons(FoodPortionLifecycleSnapshot $snapshot): array
    {
        $reasons = $this->reviewReasons($snapshot);
    ...
    private function validationFailure(FoodPortionLifecycleSnapshot $snapshot, array $reasons): CatalogLifecycleResult
    {
