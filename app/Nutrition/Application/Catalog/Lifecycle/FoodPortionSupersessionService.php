...
final class FoodPortionSupersessionService
{
    ...
    public function createSuccessor(
        ...
        return DB::transaction(function () use ($command, $context): CatalogSuccessorCreationResult {
            ...
            $predecessor = FoodPortion::query()->lockForUpdate()->find($identity->id);

            if ($reference === null || $predecessor === null || $predecessor->food_reference_id !== $reference->id) {
                throw new CatalogLifecycleSubjectNotFoundException;
            }

            $lineage = FoodPortion::query()
                ->where('lineage_id', $predecessor->lineage_id)
                ->orderBy('id')
                ->lockForUpdate()
                ->get();
            $directSuccessor = $lineage->first(
                fn (FoodPortion $portion): bool => $portion->supersedes_food_portion_id === $predecessor->id,
            );

            if ($directSuccessor !== null) {
                $directSuccessor = FoodPortion::query()->lockForUpdate()->find($directSuccessor->id);
            }
            ...
            $result = $this->policy->evaluate(
                ...
                $this->snapshot(
                    ...
                    $source,
                    $context,
                    matchingPreparationExists: true,
            ...
            try {
                $successor = FoodPortion::query()->withSavepointIfNeeded(
                    fn (): FoodPortion => FoodPortion::query()->create([
    ...
    public function activateSuccessorReplacingCurrent(
        ...
        return DB::transaction(function () use ($command, $context): CatalogActiveReplacementResult {
            ...
            $reference = FoodReference::query()->lockForUpdate()->find($identity->food_reference_id);
            $successor = FoodPortion::query()->lockForUpdate()->find($identity->id);

            if ($reference === null || $successor === null || $successor->food_reference_id !== $reference->id) {
                throw new CatalogLifecycleSubjectNotFoundException;
            }

            $predecessor = $successor->supersedes_food_portion_id === null
                ? null
                : FoodPortion::query()->lockForUpdate()->find($successor->supersedes_food_portion_id);
            $activePortions = FoodPortion::query()
                ->where('food_reference_id', $reference->id)
                ->where('locale', $successor->locale)
                ->where('portion_key', $successor->portion_key)
                ->where('preparation_key', $successor->preparation_key)
                ...
                ->lockForUpdate()
                ->get();
            $successorOfSuccessor = FoodPortion::query()
                ->where('supersedes_food_portion_id', $successor->id)
                ->lockForUpdate()
                ->first();
            $source = $successor->food_source_id === null
                ? null
                : FoodSource::query()->lockForUpdate()->find($successor->food_source_id);
            $matchingPreparationExists = $successor->preparation_key === 'any'
                || FoodReferenceVersion::query()
                    ->where('food_reference_id', $reference->id)
                    ->where('preparation_key', $successor->preparation_key)
                    ->lockForUpdate()
                    ->exists();
            ...
            $exactReplacement = $predecessor !== null
                ...
                && $activePortions->count() === 1
                && $activePortions->first()->id === $predecessor->id;
            $result = $this->policy->evaluate(
                $command,
                $this->snapshot(
                    $successor,
                    ...
                    $source,
                    $context,
                    matchingPreparationExists: $matchingPreparationExists,
            ...
            try {
                FoodPortion::query()->withSavepointIfNeeded(function () use ($predecessor, $successor, $command, $context): void {
                ...
                return $this->storeReplacementFailure(
                    ...
                    CatalogLifecycleResult::conflict(
                        CatalogLifecycleReason::ActivePortionConflict,
    ...
    private function snapshot(
        ...
        return new FoodPortionLifecycleSnapshot(
            ...
            contentComplete: $this->nonblank($subject->portion_key)
                && $this->nonblank($subject->display_label)
                && $this->nonblank($subject->normalized_label)
                && $this->nonblank($subject->locale)
                && $this->nonblank($subject->portion_type)
                && $this->nonblank($subject->unit_code)
            ...
            hasActivePortionConflict: $hasActiveConflict,
    ...
    /** @param Collection<int, FoodPortion> $lineage */
    private function creationReplay(
        ...
        if ($successor === null
            ...
            || $successor->supersedes_food_portion_id !== $predecessor->id) {
            throw $this->integrityFailure();
        }
    ...
    private function validateInvocation(
        ...
        if ($command->subjectType !== CatalogLifecycleSubjectType::Portion || $command->operation !== $operation) {
            throw new InvalidArgumentException('The lifecycle command does not match the portion supersession operation.');
        }
    ...
    private function creationConflictReason(UniqueConstraintViolationException $exception): ?CatalogLifecycleReason
    {
        ...
        return match (true) {
            str_contains($detail, 'food_portions_supersedes_food_portion_id_unique') => CatalogLifecycleReason::SuccessorExists,
            str_contains($detail, 'food_portions_lineage_id_revision_number_unique') => CatalogLifecycleReason::NumberConflict,
            str_contains($detail, 'food_portions_public_id_unique') => CatalogLifecycleReason::ConcurrencyConflict,
            default => null,
        };
    }
    ...
    private function isActiveUniqueConflict(UniqueConstraintViolationException $exception): bool
    {
        return $this->sqlState($exception) === '23505'
            && str_contains($this->driverDetail($exception), 'food_portions_one_active_key_unique');
    }
